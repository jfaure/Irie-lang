...
mkSTAlts alts = do
  mkStruct (Just (rawStructType stAlts)) [mkNullPtr tySplitTree , tagSTAlts , alts]

mkSTZipper fn args = mkStruct (Just (rawStructType stAlts))
...
mkSplitTree retTy label labelMap _defaultFn = let
  mkAlt (CoreFn ars free term ty) = do
    let iFree = IS.toList free
        testRetTy r = case r of { LT.PointerType (L.NamedTypeReference (L.Name "A")) _ -> True ; _ -> False }
        isDataFn = testRetTy retTy -- isLabelTy retTy -- SORTOF HACK
...
genEvalSplitTree retPtr st label = do
  ...
  stNext <- load' =<< (st `gep` (constI32 <$> [0 , 0]))

  -- branchTable
  let altsBranch = do
        stAlts <- (`loadIdx` 2) =<< bitcast st (structAliasType stAlts)
...
evalSplitTree' retPtr st l = do
  ...
  l'       <- bitcast l tyLabel
  nullST   <- icmp IP.EQ st (L.ConstantOperand $ C.Null tySplitTree)
  mdo
    condBr nullST end doEval
    doEval <- block `named` "doEval"
