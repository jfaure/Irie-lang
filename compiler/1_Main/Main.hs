...
-- also stop when requested work is done
data Pipeline
  = KONoFile FilePath
  | KOParse  (ParseErrorBundle Text Void)
  | KOImports BitSet
  | KOJudge Errors
  | OKParse P.Module
  | OKJudge JudgedModule
...
inferResolve flags fName modIName modResolver modDeps parsed progText maybeOldModule = let
  ...
  (tmpResolver  , exts) = resolveImports
    ...
    maybeOldModule
  (judgedModule , errors) = judgeModule nBinds parsed (deps modDeps) modIName 0 hNames exts srcInfo
  JudgedModule _modIName _modNm _nArgs bindNames _a _b judgedBinds _specs = judgedModule
...
handleJudgedModule (flags , fName , judgedModule , newResolver , _exts , errors , srcInfo) = let
  JudgedModule modI modNm nArgs bindNames a b judgedBinds _specs = judgedModule
